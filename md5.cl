...
void md5_multiBlock(uint *hash, MD5_LOCAL const uint *key, const uint len)
{
    // accept any size key
    uint i, j;
	uint W[16];
...
void md5_multiBlock_local(uint hash[4], __local const uint *key, const uint len)
{
    // accept any size key
    uint i, j;
	uint W[16];
 ...
 __kernel void md5_general(__global uint *hashes, __global const uint *keys, const uint len)
{
    // accept any size key len
	...
	const uint keyLen = (len+3)/4;         // length of key in 32-bit ints
	const uint keyOffset = gid * keyLen;   // offset of the key in global array
    const uint alignedLen = (len/64) * 16; // length of key in 32-bit ints, rounded to block size
    
	keys += keyOffset;
	
    uint4 v = {0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476 };
 ...
 __kernel void md5_discrete(__global uint *hashes, __global const uint *keys, const uint len)
{
    ...
    if (len < 56)
    {
        ...
        PUTCHAR(W, len, 0x80); // append "1" bit to the message
        W[14] = len << 3;      // last 8 bytes/64-bits is message length in bits
    ...
    else if (len < 120)
    {
        ...
        PUTCHAR(W, len-64, 0x80); // append "1" bit to the message
        W[14] = len << 3;
        v = md5_block(W, v);
    }
...
__kernel void md5_indexed(__global uint *hashes, __global const uint *keys, __global const uint *index)
{
	uint gid = get_global_id(0);
	uint W[16] = { 0 };
	uint i;
    ...
    // key length is lower 6 bits == 63 bytes max, however we're limited to 55
	uint len = base & 63;
	...
	hashes[gid * 4 + 0] = a + 0x67452301;
	hashes[gid * 4 + 1] = b + 0xefcdab89;
	hashes[gid * 4 + 2] = c + 0x98badcfe;
	hashes[gid * 4 + 3] = d + 0x10325476;
}
#endif
